-- question statement
    -- Find the percentage increase in covid cases for each month
...
with monthly_cases as (
    select
        date_part('month', record_date) as case_month,
        sum(cases_count) as monthly_case
    from covid_cases
    group by date_part('month', record_date)),
total_cases as (
    select
        *,
        sum(monthly_case) over(order by case_month) as total_case
    from monthly_cases)
select
	*,
